...
import { generateForms } from "@kie-tools/form-generation-tool/dist/generation";
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("extension.kogito.generateForms", async (args: any) => {
      ...
      if (projectUri && projectUri[0]) {
        ...
        const style = await vscode.window.showQuickPick(["Bootstrap", "Patternfly"], {
        ...
        vscode.window.showInformationMessage(`your choice: ${style}`);
        ...
        if (filesThatWillBeOverriden.length === 0) {
          ...
          generateForms({ path: projectPath, overwrite: false, type: style.toLowerCase() });
          ...
          if (override === "Override") {
            generateForms({ path: projectPath, overwrite: true, type: style.toLowerCase() });
          }
