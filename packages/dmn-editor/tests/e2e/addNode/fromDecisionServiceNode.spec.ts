...
test.describe("Add node - from Decision Service node", () => {
  test.beforeEach(async ({ palette }) => {
    await palette.dragNewNode({ type: NodeType.DECISION_SERVICE, targetPosition: { x: 100, y: 100 } });
  });

  test("should add connected Decision node from Decision Service node", async ({ diagram, nodes, edges }) => {
  ...
  test("should add connected BKM node from Decision Service node", async ({ diagram, nodes, edges }) => {
    await nodes.dragNewConnectedNode({
      from: DefaultNodeName.DECISION_SERVICE,
      type: NodeType.BKM,
      targetPosition: { x: 500, y: 500 },
  ...
  test("should add connected Text Annotation node from Decision Service node", async ({ diagram, nodes, edges }) => {
    await nodes.dragNewConnectedNode({
      from: DefaultNodeName.DECISION_SERVICE,
