...
test.describe("Add node - from Knowledge Source node", () => {
  ...
  test("should add connected Decision node from Knowledge Source node", async ({ diagram, nodes, edges }) => {
  ...
  test("should add connected BKM node from Knowledge Source node", async ({ diagram, nodes, edges }) => {
    await nodes.dragNewConnectedNode({
      from: "Knowledge Source - A",
      type: NodeType.BKM,
      targetPosition: { x: 100, y: 300 },
  ...
  test("should add connected Knowledge Source node from Knowledge Source node", async ({ diagram, nodes, edges }) => {
    await nodes.dragNewConnectedNode({
      from: "Knowledge Source - A",
      type: NodeType.KNOWLEDGE_SOURCE,
      targetPosition: { x: 100, y: 300 },
  ...
  test("should add connected Text Annotation node from Knowledge Source node", async ({ diagram, nodes, edges }) => {
    test.skip(true, "");
