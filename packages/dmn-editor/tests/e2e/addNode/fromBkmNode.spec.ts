...
test.describe("Add node - from BKM node", () => {
  ...
  test("should add connected Decision node from BKM node", async ({ diagram, nodes, edges }) => {
  ...
  test("should add connected BKM node from BKM node", async ({ diagram, nodes, edges }) => {
    await nodes.dragNewConnectedNode({
      from: "BKM - A",
      type: NodeType.BKM,
      targetPosition: { x: 100, y: 300 },
  ...
  test("should add connected Text Annotation node from BKM node", async ({ diagram, nodes, edges }) => {
    await nodes.dragNewConnectedNode({
      from: "BKM - A",
