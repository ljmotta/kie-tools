...
test.describe("Add node - from Input Data node", () => {
  test.beforeEach(async ({ palette }) => {
    await palette.dragNewNode({ type: NodeType.INPUT_DATA, targetPosition: { x: 100, y: 100 } });
  });

  test("should add connected Decision node from Input Data node", async ({ diagram, nodes, edges }) => {
  ...
  test("should add connected Knowledge Source node from Input Data node", async ({ diagram, nodes, edges }) => {
    await nodes.dragNewConnectedNode({
      from: DefaultNodeName.INPUT_DATA,
  ...
  test("should add connected Text Annotation node from Input Data node", async ({ diagram, nodes, edges }) => {
    await nodes.dragNewConnectedNode({
      from: DefaultNodeName.INPUT_DATA,
