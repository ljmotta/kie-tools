...
test.describe("Decision Service Node - Delete", () => {
  test("should delete Decision Service node", async ({ palette, nodes }) => {
  ...
  test.describe("Nested nodes", () => {
    test("should delete the Decision Service node", async ({ palette, nodes }) => {
      await palette.dragNewNode({ type: NodeType.DECISION_SERVICE, targetPosition: { x: 100, y: 100 } });
