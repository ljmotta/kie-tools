...
export function EditorPage() {
  ...
  const { workspaceId: optionalWorkspaceId, fileRelativePath, extension } = useParams();
  ...
  const fileRelativeCompletePath = `${fileRelativePath}.${extension}`;
  ...
  useEffect(() => {
    document.title = `${env.KIE_SANDBOX_APP_NAME} :: ${fileRelativeCompletePath}`;
  }, [env.KIE_SANDBOX_APP_NAME, fileRelativeCompletePath]);
...
Error details: ${err}`);
  ...
  const settingsAwareEditorEnvelopeLocator = useMemo(() => {
    if (settings.editors.useLegacyDmnEditor && fileRelativeCompletePath.endsWith(".dmn")) {
  ...
  }, [editorEnvelopeLocator, fileRelativeCompletePath, settings.editors.useLegacyDmnEditor]);
  ...
  return (
    <OnlineEditorPage onKeyDown={onKeyDown}>
      <PromiseStateWrapper
        ...
        rejected={(errors) => (
          <EditorPageErrorPage title={"Can't open file"} errors={errors} path={fileRelativeCompletePath} />
        )}
        resolved={(file) => (
