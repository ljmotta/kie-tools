      ...
      const importIndex: ImportIndex = {
        ...
        models: new Map(
          resources.map((r) => [
            ...
            {
              xml: r.xmlContent,
              definitions: getMarshaller(r.xmlContent || EMPTY_DMN(), { upgradeTo: "latest" }).parser.parse()
                .definitions,
            },
          ])
        ),
...
Error details: ${error}`);
  ...
  public getPathFromNodeId(resources: DmnLanguageServiceResource[], nodeId: string): string {
    for (const resourceContent of resources) {
      const xmlContent = this.parser.parseFromString(resourceContent.xmlContent ?? "", XML_MIME);
      const inputDataTag = this.inputDataRegEx.exec(resourceContent.xmlContent ?? "");
