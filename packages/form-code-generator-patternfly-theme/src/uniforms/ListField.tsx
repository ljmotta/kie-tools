...
import { connectField, context, HTMLFieldProps, joinName, useField } from "uniforms/cjs";
import { getInputReference, getStateCode, renderField } from "./utils/Utils";
import { codeGenContext, useAddFormElementToContext } from "./CodeGenContext";
import { FormInput, InputReference } from "../api";
import { ARRAY } from "./utils/dataTypes";
import { renderListItemFragmentWithContext, renderNestedInputFragmentWithContext } from "./rendering/RenderingUtils";
...
const List: React.FC<ListFieldProps> = (props: ListFieldProps) => {
  ...
  const item = renderListItemFragmentWithContext(
    uniformsContext,
    "$",
    {
      itemProps: {
  ...
  const jsxCode = `<div fieldId={'${props.id}'}>
      ...
      <div>
        {${ref.stateName}?.map((_, itemIndex) =>
          ...
          >
            <div style={{ width: '100%', marginRight: '10px' }}>${item?.jsxCode}</div>
            <div>
              <Button
  ...
  // TODO ADD PLUS AND MINUS
  const element: FormInput = {
    ref,
    pfImports: [...new Set(["Split", "SplitItem", "Button", ...(item?.pfImports ?? [])])],
    reactImports: [...new Set([...(item?.reactImports ?? [])])],
    requiredCode: [...new Set([...(item?.requiredCode ?? [])])],
    jsxCode,
