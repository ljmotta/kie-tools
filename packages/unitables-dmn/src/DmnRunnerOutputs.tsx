          ...
          decisionResult.forEach(({ decisionName }) => {
            ...
            if (outputField && isOutputWithInsideProperties(outputField)) {
              ...
              outputFieldsMap.set(decisionName, {
                ...
                insideProperties: insideProperties,
...
function deepFlatInsidePropertiesOutputs(
  outputFieldsMap: Map<string, OutputFields>,
  decisionName: string,
  outputField: OutputWithInsideProperties
): Array<OutputFields> {
  const flattenOutputFields = outputField.insideProperties.flatMap((insideOutputField: OutputFields) => {
    if (isOutputWithInsideProperties(insideOutputField)) {
      return insideOutputField.insideProperties;
    }
  });
  return flattenOutputFields
    .flatMap((outputField) => {
      if (outputField && isOutputWithInsideProperties(outputField)) {
        return deepFlatInsidePropertiesOutputs(outputFieldsMap, decisionName, outputField);
      }
      return outputField;
    })
    .filter((e) => e !== undefined) as Array<OutputFields>;
}
